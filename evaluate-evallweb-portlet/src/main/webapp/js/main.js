...
var checkFormatFunctions = [$.Deferred().resolve()];
    ...
    $('#fileupload').fileupload({
        ...
        progress: function (e, data) {
        	...
        	if(data.result.status == -1) {
            	...
            	var ajaxDeferred = checkFormatFunctions[checkFormatFunctions.length - 1];

            	var checkFunction = function (index) { return checkFileFormat(tr.closest('table'), index, false, OUTPUT_TYPE, function voidFunction(){}, 36002);}

            	if(data.originalFiles.indexOf(data.files[0]) == data.originalFiles.length - 1) {

            		hide_info(e, 'uploading_file');

            		checkFunction = function (index) { return checkFileFormat(tr.closest('table'), index, true, OUTPUT_TYPE, checkEvaluateButton, 36002);}
            	}

            	var deferred = $.when(ajaxDeferred).then(function () { return checkFunction(index);});

            	if(data.originalFiles.indexOf(data.files[0]) == data.originalFiles.length - 1) {

            		checkFormatFunctions = [$.Deferred().resolve()];

            	} else {

            		checkFormatFunctions.push(deferred);
            	}
			...
			success: function(data) {
				...
				if(data.status == -1) {
					...
					$('#uploaded-files > tbody > tr:not(.head)').first().before(
		        			$('<tr class="fileTr" ' + SYSTEM_ORDER_PARAMETER + '="' + index + '" target="' + data.content.fileId + '"/>')
		        			.append($('<td class="voidTd"></td>'))
	            	...
	            	$('#screen-block').animate({opacity: '0'}, 750, function() {
		            	...
		            	var checkFunction = function (index) { return checkFileFormat(tr.closest('table'), index, true, OUTPUT_TYPE, checkEvaluateButton, origin);}
