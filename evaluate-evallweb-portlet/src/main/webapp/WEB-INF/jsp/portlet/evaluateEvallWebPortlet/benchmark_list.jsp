				...
				<div class="frameSlide">
					<div class="benchmarkSearchBarZone">
						<div class="benchmarkSearchBar">
							<div class="benchmark_clear_search" onclick="clearSearchBenchmark(event, this, 11009);">
							...
							<div class="benchmark_search">
								...
								<input type="submit" class="button_benchmark_search" onclick="copyBenchmarkSearchText(this); searchBenchmark(event, this, $('#evall_benchmarks'), 11008);" value="">
							</div>
						</div>
					</div>
					<div class="benchmark_zone subzone">
							...
							<table class="table-main dark removeBorders">
								<tbody>
									...
									<c:forEach var="benchmark" items="${requestScope[benchmarksParameter]}" varStatus="loop">
										...
										<td class="benchmark content selection ${position} ${isSelected}" onClick="if(!$(this).hasClass('selected')){select(event, this); setBenchmark(event, this, 11001);}" target="${benchmark.getId()}">
											<div class="col_label">
												<div class="col_label_content">
													...
													<c:choose>
														<c:when test="${benchmark.getWorkshop().length() > 36}">
															<div onmouseover="showPopupDiv(event, this);" onmouseout="hidePopupDiv(event, this);">
																...
																<div onclick="event.stopPropagation(); event.preventDefault();" class="popupDiv">${benchmark.getWorkshop()}</div>
															</div>
														</c:when>
														<c:otherwise>
													...
													<img src="${iconUrl}" />
													<div class="detail moreButton" onClick="goToDescription(event, this, 11002);"></div>
												</div>
											</div>
										</td>
										<c:choose>
						...
						<c:if test="${fn:length(requestScope[benchmarksParameter]) > rowsPerPageBenchmark * columnsPerRowBenchmark}">
							<div class="moveBenchmarkView moveRows">
								<div class="up upContent2 disable" onClick="upContent(event, this, ${rowsPerPageBenchmark}, true, 11003);"></div>
								<div class="up upContent disable" onClick="upContent(event, this, ${rowsPerPageBenchmark}, false, 11004);"></div>
								<div class="down downContent" onClick="downContent(event, this, ${rowsPerPageBenchmark}, false, 11005);"></div>
								<div class="down downContent2" onClick="downContent(event, this, ${rowsPerPageBenchmark}, true, 11006);"></div>
							</div>
						</c:if>
					</div>
		   		</div>
	   		...
	   		<div class="arrow ${arrowState}">
				<div class="blockArrow"></div>
		   		<div class="frameArrow">
					<a class="moveSectionDown" href="javascript:void(0)" onclick="unblock_evaluation_option(event, true, 11000);">
