...
<div class="description frameSlide" id="benchmark_description">
	<div class="back">
		<a href="javascript:void(0)" class="title blackLink" id="titleBenchmark" onClick="backSlide(event, 11100);" target="${requestScope[benchmarkParameter].getId()}">${back}</a>
	</div>
	<div class="description_zone subzone">
		<table class="table_description">
			...
			<tr>
				...
				<td class="links">
					<c:if test="${!empty requestScope[benchmarkParameter].getUrlPage()}"><p><a class="standardColor bold official_website" href="javascript:void(0)" onclick="openURL(event, this, false, 'benchmark', 'viewPage', '${requestScope[benchmarkParameter].getId()} | ${requestScope[benchmarkParameter].getName()}', 11101);" target="${requestScope[benchmarkParameter].getUrlPage()}">${benchmark_website}</a></p></c:if>
					<c:if test="${!empty requestScope[benchmarkParameter].getUrlPaper()}"><p><a class="standardColor bold paper" href="javascript:void(0)" onclick="openURL(event, this, false, 'benchmark', 'viewPaper', '${requestScope[benchmarkParameter].getId()} | ${requestScope[benchmarkParameter].getName()}', 11102);" target="${requestScope[benchmarkParameter].getUrlPaper()}">${benchmark_paper}</a></p></c:if>
					<c:if test="${requestScope[benchmarkParameter].isHasBibtex()}"><p><a class="standardColor bold bibtex" href="javascript:void(0)" onclick="openURL(event, this, false, 'benchmark', 'downloadBibtex', '${requestScope[benchmarkParameter].getId()} | ${requestScope[benchmarkParameter].getName()}', 11103);" target="${DownloadServiceURL}">${benchmark_bibtex}</a></p></c:if>
				</td>
			</tr>
			<tr>
				<td colspan="3">
